...
public class MySlidingDrawer extends RelativeLayout {
    ...
    private boolean mHideAfterOpend;
    ...
    private int bottomTouchOffset;
    private int noTouchScope;

    public void setNoTouchScope(int noTouchScope) {
        this.noTouchScope = noTouchScope;
    }
    ...
    public MySlidingDrawer(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.mHideAfterOpend = a.getBoolean(R.styleable.MySlidingDrawer_handle_hide, false);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mVertical) {
            ...
            int width = 0;
            if(mHideAfterOpend){
            	width = widthSpecSize - mTopOffset;
            }else{
        ...
        if (mVertical) {
            ...
            if(mHideAfterOpend){
	            content.layout(mTopOffset, 0,
	            		mTopOffset + content.getMeasuredWidth() + childWidth,
	            		content.getMeasuredHeight() + childWidth);
            }else{
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        if(mHideAfterOpend){
	        mContent.getHitRect(frame);//哪个控件拦截消息
	        
	        if (!mTracking && !frame.contains((int) x, (int) y)) {
	            return false;
	        }
	
	        //展开状态下 把手右边100范围内触碰拦截
	        if(mExpanded && x > 100 + handle.getLeft()){
	        	return false;
	        }

            //收缩状态下 最右侧以左40范围内触碰拦截
            if (!mExpanded && (y > noTouchScope-50 && y < noTouchScope+20)) {
                return true;
            }

	        //收缩状态下 最右侧以左40范围内触碰拦截
	        if(!mExpanded && x < mContent.getMeasuredWidth() - 40){
	        	return false;
	        }
	        
	        //收缩状态下 最下边以上60范围内不做事件处理
	        if(!mExpanded && y > mContent.getMeasuredHeight() - bottomTouchOffset){
	        	return false;
	        }
        }else{
        	mHandle.getHitRect(frame);//哪个控件拦截消息
                ...
                case MotionEvent.ACTION_CANCEL: {
                    ...
                    if (Math.abs(velocity) < mMaximumTapVelocity) {
                        if (vertical ? (mExpanded && top < mTapThreshold + mTopOffset) ||
                                (!mExpanded && top > mBottomOffset + getMyBottom() - getMyTop() -
                                        mHandleHeight - mTapThreshold) :
                                (mExpanded && left < mTapThreshold + mTopOffset) ||
                                (!mExpanded && left > mBottomOffset + getMyRight() - getMyLeft() -
    ...
    private void moveHandle(int position) {
        ...
        if (mVertical) {
            if (position == EXPANDED_FULL_OPEN) {
                ...
                handle.offsetTopAndBottom(mBottomOffset + getMyBottom() - getMyTop() -
                ...
                } else if (deltaY > mBottomOffset + getMyBottom() - getMyTop() - mHandleHeight - top) {
                    deltaY = mBottomOffset + getMyBottom() - getMyTop() - mHandleHeight - top;
                }
            ...
            if (position == EXPANDED_FULL_OPEN) {
                ...
                handle.offsetLeftAndRight(mBottomOffset + getMyRight() - getMyLeft() -
                ...
                } else if (deltaX > mBottomOffset + getMyRight() - getMyLeft() - mHandleWidth - left) {
                    deltaX = mBottomOffset + getMyRight() - getMyLeft() - mHandleWidth - left;
                }
    ...
    private void prepareContent() {
        ...
        if (content.isLayoutRequested()) {
            if (mVertical) {
                ...
                int height = getMyBottom() - getMyTop() - childHeight - mTopOffset;
                content.measure(MeasureSpec.makeMeasureSpec(getMyRight() - getMyLeft(), MeasureSpec.EXACTLY),
                        MeasureSpec.makeMeasureSpec(height, MeasureSpec.EXACTLY));
                content.layout(0, mTopOffset + childHeight, content.getMeasuredWidth(),
                        mTopOffset + childHeight + content.getMeasuredHeight());
            } else {
                final int childWidth = mHandle.getWidth();
                int width = getMyRight() - getMyLeft() - childWidth - mTopOffset;
                content.measure(MeasureSpec.makeMeasureSpec(width, MeasureSpec.EXACTLY),
                        MeasureSpec.makeMeasureSpec(getMyBottom() - getMyTop(), MeasureSpec.EXACTLY));
...
//        RelativeLayout.LayoutParams lp2 = (LayoutParams) mContent.getLayoutParams();
//        
//        RelativeLayout.LayoutParams lp = (LayoutParams) mHandle.getLayoutParams();
//        lp.leftMargin = 0;
//        lp.topMargin = lp2.topMargin;
//        lp.bottomMargin = lp2.bottomMargin;
//        
//        
...
//      //  mHandle.invalidate();
	...
	public void setBottomTouchOffset(int bottomTouchOffset) {
		this.bottomTouchOffset = bottomTouchOffset;
	}
