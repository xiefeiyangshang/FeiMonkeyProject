package com.whatyplugin.base.adaper;
...
public class BaseAdapterHelper {
	...
	static BaseAdapterHelper get(Context context, View convertView, ViewGroup parent, int layoutId, int position) {
		BaseAdapterHelper helper = convertView == null ? new BaseAdapterHelper(context, parent, layoutId, position) : (BaseAdapterHelper) convertView.getTag();
		return helper;
	}
    ...
    public BaseAdapterHelper setAlpha(int viewId, float value) {
        if(Build.VERSION.SDK_INT >= 11) {
            ...
            this.retrieveView(viewId).startAnimation(animation);
        }
    ...
    public BaseAdapterHelper setImageUrl(int viewId, String url, ImageLoader imageLoader, int width, int height, boolean isNeedScaled, ImageType type, MCImageHandleInterface config) {
    	MCImageView view =  (MCImageView) this.retrieveView(viewId);//自定义的image
        view.setImageUrl(url, imageLoader, width, height, isNeedScaled, type, config);
        return this;
    }
